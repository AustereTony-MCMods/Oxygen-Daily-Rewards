...
public class DailyRewardsArgumentOperator implements ArgumentExecutor {
            ...
            if (args.length == 2) {
                ...
                if (args[1].equals("-open-menu")) {
                    if (args.length == 3) {
                        ...
                        OxygenMain.network().sendTo(new CPOpenDailyRewardsMenu(), targetPlayerMP);
                    }
    ...
    public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, @Nullable BlockPos targetPos) {
        if (args.length == 2)
            return CommandBase.getListOfStringsMatchingLastWord(args, "-open-menu", "-reload-rewards", "-reset-latest-reward", "-set-days-rewarded");
